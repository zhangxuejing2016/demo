...
public class DateUtils {
    ...
    public static final Date getNextDays(int days){
        Calendar cal = GregorianCalendar.getInstance();
        return new GregorianCalendar(cal.get(Calendar.YEAR), cal
                .get(Calendar.MONTH), cal.get(Calendar.DAY_OF_MONTH) - days)
                .getTime();
    }
     ...
     * @return 偏移后的时间串
     */
    public static Date getChangeDateAddTime(Date date, Date time, String type,
                                            int iQuantity) {
        ...
        int hour = Integer.parseInt(DateUtils.formatDate(time, "HH"));
        int mi = Integer.parseInt(DateUtils.formatDate(time, "mm"));
        int ss = Integer.parseInt(DateUtils.formatDate(time, "ss"));
...
//        } catch (ParseException e) {
//            String msg = String.format("Exception when try to timeSubtract date1 %s, date2 %s: %s", date1, date2, e.getMessage());
...
//    }
    ...
    public static Date getFistYearAgo(Date dt, Long n) {
        ...
        return new GregorianCalendar(cal.get(Calendar.YEAR) - n.intValue(), 0,
                1).getTime();
    ...
    public static Date getLastYearAgo(Date dt, Long n) {
        ...
        return new GregorianCalendar(cal.get(Calendar.YEAR) - n.intValue(), 11,
                31).getTime();
